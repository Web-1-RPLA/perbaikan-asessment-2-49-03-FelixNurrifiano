...
<body>
    ...
    <div class="content">
        ...
        <div class="menu-kamar">
            <a href="index.php" class="btn-link">← Kembali ke Home</a>
            <a href="tambahkamar.php" class="btn-link" style="background-color: #00a4f0; color: white; border-color: #00a4f0;">+ Tambah Kamar Baru</a>
        </div>
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo "<td>Rp " . number_format($row['harga_per_malam'], 0, ',', '.') . "</td>";
                echo "<td>" . htmlspecialchars($row['status']) . "</td>";
                echo "<td style='text-align: center;'>
                        <a href='editkamar.php?id=" . $row['id'] . "' class='action-btn edit-btn'>Edit</a> | 
                        <a href='hapuskamar.php?id=" . $row['id'] . "' class='action-btn hapus-btn' onclick='return confirm(\"Apakah Anda yakin ingin menghapus kamar nomor " . htmlspecialchars($row['nomor_kamar']) . "?\")'>Hapus</a>
                      </td>";
                echo "</tr>";
            }
            echo "</table>";
        } else {
            echo "<div class='pesan-kosong'>Belum ada data kamar di database.</div>";
        }
        ?>
    </div>
